...
/// <summary>
///     Перечисление возможных значений ячейки.
///     Названия (кроме None и Null) по степеням двойки
/// </summary>
...
public class Cell : OnPropertyChangedClass
{
    ...
    /// <summary>
    ///     Флаг - значение ячейки получено вычислением.
    ///     Служит для предотвращения двойной обработки ячейки в одном шаге.
    ///     <see langword="true" /> если значение получено слиянием в текущем шаге
    /// </summary>
    ...
    public Cell MoveTo(Cell target)
    {
        if (
            ...
            || (target.Value != CellValueEnum.None &&
                target.Value != Value) // если значение цели не None и не равно значению ячейки
        )
            return null;
        ...
        {
            var newValue = (int)target.Value + (int)Value;
